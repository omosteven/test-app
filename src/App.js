...
const App = () => {
    ...
    const fetchChatSetting = async () => {
        try {
            ...
            if (res.status === 200) {
                ...
                root.style.setProperty('--default-primary-color', chatThemeColor);
                sayFetching(false)
            }
    ...
    if (fetchingError) return <div>{fetchingError} <span onClick={() => fetchChatSetting()} style={{ color: "blue", marginLeft: "10px", cursor: 'pointer' }}>Retry ?</span></div>
